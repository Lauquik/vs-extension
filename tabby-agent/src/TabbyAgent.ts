...
export class TabbyAgent extends EventEmitter implements Agent {
  ...
  public async chatCompletion(
    ...
    this.logger.trace({ request }, "Call chatCompletion");
    let chatResponse: ChatResponse;
    try {
        if (!this.api) {
            throw new Error("http client not initialized");
        }
        const requestPath = "/v1/chat";
        const requestOptions = {
            body: {
              userQuery: request.userQuery,
              selectedContent: request.selectedContent,
              entireContent: request.entireContent,
        ...
        this.logger.debug(
            { requestOptions, url: this.config.server.endpoint + requestPath },
            "Chat request",
        );
        ...
        if (isTimeoutError(error)) {
            this.logger.debug({ error }, "Chat request timeout");
        } else if (isCanceledError(error)) {
            this.logger.debug({ error }, "Chat request canceled");
        } else {
            this.logger.error({ error }, "Chat request failed with unknown error");
